...
입력 검증 또는 비즈니스 규칙 위반이 `/api/**`에서 발생하면 JSON으로 반환합니다. 일반 화면 요청의 오류는 FitFlow HTML 오류 화면으로 처리합니다.

## 인증과 권한

FitFlow는 세션 기반 인증을 사용합니다. 로그인과 로그아웃은 JSON API가 아닌 다음 MVC 경로에서 처리합니다.
