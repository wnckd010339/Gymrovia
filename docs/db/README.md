...
- 출석 시간대 구분: `V2__add_attendance_time_zone.sql` (`attendances.time_zone` 추가)
...
```text
V3__add_member_note.sql
V4__add_payment_index.sql
```
...
- 새 출석의 날짜·입실·퇴실은 `Asia/Seoul` 기준이며 `time_zone='Asia/Seoul'`로 구분합니다.
- V2는 기존 출석 시각과 날짜를 수정하지 않습니다. 기존 행의 `time_zone`은 NULL(기준 미확인)입니다.
- 기존 미퇴실 행은 종전 JVM 시간 기준으로 퇴실 처리합니다. JVM/컨테이너 시간대를 함께 변경하지 마세요.
- 기존 기록에는 샘플 한국 시간과 서버 UTC 기록이 섞일 수 있습니다. 확인된 행만 백업 후 별도로 보정해야 합니다. NULL 행 전체에 9시간을 더하거나 시간대를 일괄 지정하지 않습니다.
- 따라서 배포 후 새 기록은 한국 시간으로 보이지만, 기존 `06:20` 기록은 자동으로 `15:20`이 되지 않습니다.
- 운영 배포 전에 RDS 스냅샷/백업을 확인하세요. 새 앱 시작 시 Flyway V2가 자동 적용되므로 수동 ALTER나 V1 재실행은 필요하지 않습니다. 앱 DB 계정에는 ALTER 권한이 필요합니다.
- V2 이후 구버전 앱으로 롤백하면 새 출석 시간 규칙을 알지 못합니다. 입실 중인 새 기록이 있는 상태에서 구버전으로 되돌리지 말고 출석 기능을 중지한 뒤 별도로 복구 계획을 세우세요.
- 관리자·회원 출석 날짜, 대시보드 출석 집계, 출석 월별 집계·통계 기본 기간을 한국 기준으로 계산합니다. 기존 기록의 날짜는 별도 보정 전까지 원래 값으로 집계됩니다.
- 결제·예약 등 다른 도메인의 저장 시간 정책은 이번 변경 대상이 아닙니다.
